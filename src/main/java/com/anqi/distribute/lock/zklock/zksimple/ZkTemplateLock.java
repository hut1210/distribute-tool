...
public class ZkTemplateLock extends AbstractTemplateLock {
    ...
    private static Logger logger = LoggerFactory.getLogger(ZkTemplateLock.class);

    private ZkClient client;

    public ZkTemplateLock() {
        client = new ZkClient(zkServers, sessionTimeout, connectionTimeout);
        logger.info("zk client 连接成功:{}",zkServers);
    }
    ...
    protected boolean tryLock() {
        try {
            ...
            logger.error(Thread.currentThread().getName()+"创建失败");
            return false;
        }
        return true;
    }
